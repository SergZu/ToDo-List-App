...
const EditTask = function(props : EditTaskProps) {
    ...
    const [ taskTextError, setTaskError] = useState(false);
     ...
     const onChangeTextHandler : React.ChangeEventHandler<HTMLTextAreaElement> = (evt) => {
        ...
        const taskText  = evt.currentTarget.value.length < 150 ? 
                                                                    evt.currentTarget.value : 
                                                                    `${evt.currentTarget.value.slice(0, 150)}...`;
    ...
    const onChangeCategoryHandler : React.ChangeEventHandler<HTMLInputElement> = (evt) => {
        const newData = {...taskData};
        const taskCategory = evt.currentTarget.value.length < 40 ? 
                                                                    evt.currentTarget.value : 
                                                                    `${evt.currentTarget.value.slice(0, 40)}...`;
    ...
    const dataInputValue = taskData.expiredDate !== '' ? 
                                                         convertDate(taskData.expiredDate as number) : 
                                                         '';
    console.log(`Expired Date : ${taskData.expiredDate}`)
    const categoryInputValue = taskData.category !== '' ? 
                                                          taskData.category : 
                                                          '';
    const importantBtnClass = taskData.important ? 
                                                   'taskEdit-btn__important highlight' : 
                                                   'taskEdit-btn__important';
    const taskTextareaClass = taskTextError ? 
                                              'taskEdit-text invalid' : 
                                              'taskEdit-text';
    const expiredDateLayout = scheduled ? 
        (<label className='taskEdit-expired'>Expired date : 
            <input 
                    ...
                    min={convertDate(Number(currentDate))} />
         </label>) 
         : (<input 
                    type='button' 
                    className='taskEdit-btn__schedule' 
    ...
    return (
        <form className='task-editable'>
            ...
            <input 
                    ...
                    onClick={onImportantClickHandler} 
                    value='Important' 
             />
                {expiredDateLayout}
            <input 
                    type='text' 
                    className='taskEdit-category' 
            ...
            <input 
                    type='button' 
                    className='taskEdit-btn__cancel' 
                    onClick={cancelTask} 
            />
            <input 
                    type='button' 
                    className='taskEdit-btn__apply' 
                    onClick={submitTask} 
            />
        </form>
    )
